...
def test_huggingfacehub_source(tmpdir):
    hfhub_model = kipoi.get_model("Basset", source="kipoi-hfhub")
    hfhub_dl_kwargs = hfhub_model.default_dataloader.example_kwargs
    tsv_tmpfile_metadata =  str(tmpdir.mkdir("hfhub").join("out_with_metadata_hfhub.tsv"))
    hfhub_model.pipeline.predict_to_file(tsv_tmpfile_metadata, hfhub_dl_kwargs, keep_metadata=True)
    preds_and_metadata_hfhub = pd.read_csv(tsv_tmpfile_metadata, sep='\t')
    assert 'metadata/ranges/chr' in preds_and_metadata_hfhub.columns 
    assert 'preds/100' in preds_and_metadata_hfhub.columns
    assert len(preds_and_metadata_hfhub['metadata/ranges/chr']) == 10
    assert len(preds_and_metadata_hfhub['preds/100']) == 10
    assert preds_and_metadata_hfhub.at[0,'preds/100'] == pytest.approx(0.4168229, rel=1e-05)

    kipoi_model = kipoi.get_model("Basset", source="kipoi")
    kipoi_dl_kwargs = kipoi_model.default_dataloader.example_kwargs
    tsv_tmpfile_metadata =  str(tmpdir.mkdir("kipoi").join("out_with_metadata_kipoi.tsv"))
    kipoi_model.pipeline.predict_to_file(tsv_tmpfile_metadata, kipoi_dl_kwargs, keep_metadata=True)
    preds_and_metadata_kipoi = pd.read_csv(tsv_tmpfile_metadata, sep='\t')
    assert preds_and_metadata_hfhub.equals(preds_and_metadata_kipoi)

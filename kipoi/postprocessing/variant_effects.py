...
def _generate_seq_sets(relv_seq_keys, dataloader, model_input, annotated_regions):
    ...
    for s_dir, allele in itertools.product(["fwd", "rc"], ["ref", "alt"]):
        k = "%s_%s" % (s_dir, allele)
        input_set[k] = {}
        for seq_key in model_input['inputs']:
            if seq_key in relv_seq_keys:
                input_set[k][seq_key] = copy.deepcopy(model_input['inputs'][seq_key])
            else:
    ...
    for seq_key in relv_seq_keys:
        ...
        # TODO: has to be validated at some point that dataloader output has the required fields!
        ...
        #
        annotated_regions["region"] = annotated_regions["chrom"] + ":" + annotated_regions["start"].astype(str) + "-" + \
                                      annotated_regions["end"].astype(str)
